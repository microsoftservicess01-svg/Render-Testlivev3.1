...
// moderation endpoint - forwards to free external API
app.post('/api/moderate-frame', async (req, res) => {
  try {
    ...
    // call external moderation endpoint
    ...
    if (!modResp.ok) {
      console.warn('moderation endpoint error', modResp.status);
      return res.json({ ok: false, modelError: true });
    }
    const j = await modResp.json();
    // expect j.result or j.label - handle flexible responses
    // treat 'nsfw' or 'sexy' as risky
    const label = (j.label || j.result || j.classification || '').toString().toLowerCase();
    const risky = label.includes('nsfw') || label.includes('sexy') || label.includes('porn') || j.nsfw === true || j.is_nsfw === true;
    if (risky) {
